...
def evaluate_oscnet_image(
    ...
    osc_files = {f for f in os.listdir(osc_dir) if f.lower().endswith(".png")}
    common_filenames = sorted(gt_files & osc_files)

    if not common_filenames:
        print("No common PNG files found in gt and osc directories.")
        return

    psnr_vals, ssim_vals = [], []
    table_rows = []
    table_header = (
            f"{'Filename':<25}{'PSNR':>12}{'SSIM':>12}\n" + "-" * 50
    )

    for filename in common_filenames:
        gt_path = os.path.join(gt_dir, filename)
        osc_path = os.path.join(osc_dir, filename)

        if not os.path.exists(gt_path) or not os.path.exists(osc_path):
        ...
        osc_img = to_grayscale(np.squeeze(plt.imread(osc_path).astype(np.float64)))

        print(f"\nProcessing {filename}:")
        print(f"GT image range: [{gt_img.min():.3f}, {gt_img.max():.3f}], shape: {gt_img.shape}")
        print(f"OSC image range: [{osc_img.min():.3f}, {osc_img.max():.3f}], shape: {osc_img.shape}")
        ...
        data_range = max(gt_img.max(), osc_img.max()) - min(gt_img.min(), osc_img.min())
        data_range = data_range if data_range > 0 else 1e-6

        psnr_val = psnr(gt_img, osc_img, data_range=data_range)
        ssim_val = ssim(gt_img, osc_img, data_range=data_range, win_size=win, channel_axis=None)
